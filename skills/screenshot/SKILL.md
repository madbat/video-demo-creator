---
name: screenshot
description: Capture screenshots from Claude Code on macOS or Windows — full screen, a specific display, an exact region by coordinates, an interactively-dragged rectangle, or a specific window (by ID/handle or by app name). Saves PNGs to a temp dir and returns the path so Claude can read the image multimodally with the Read tool. Use whenever the user asks to take a screenshot, capture the screen, grab a window, screencap something, or wants Claude to see what's on their screen — including UI debugging where Claude needs to see the actual rendered output, or when the user describes a visual problem and seeing it directly would help.
---

# Screenshot

Capture screenshots on **macOS** (built-in `screencapture`) or **Windows**
(PowerShell + the .NET/Win32 APIs), then read the resulting image back with the
Read tool so you can actually see what's on the user's screen.

## Platform: pick the right script

| Platform | Script | Invoke with |
|---|---|---|
| macOS | `scripts/capture.sh` (+ `list_windows.sh`, `list_displays.sh`) | `bash` |
| Windows | `scripts/capture.ps1` (+ `list_windows.ps1`, `list_displays.ps1`) | `powershell -ExecutionPolicy Bypass -File` |

The `.ps1` scripts mirror the `.sh` ones one-to-one (same modes, same
tab-separated output, same "print the saved PNG path on stdout" contract), so
everything below applies to both — just swap the command. On Windows the output
defaults to `%TEMP%\claude-screenshots\` (override with `CLAUDE_SCREENSHOT_DIR`).

```powershell
# Windows examples
powershell -ExecutionPolicy Bypass -File scripts\capture.ps1 full
powershell -ExecutionPolicy Bypass -File scripts\capture.ps1 display 1
powershell -ExecutionPolicy Bypass -File scripts\capture.ps1 region 0 0 800 600
powershell -ExecutionPolicy Bypass -File scripts\capture.ps1 app "Visual Studio Code"
powershell -ExecutionPolicy Bypass -File scripts\capture.ps1 window 1186566   # hwnd from list_windows.ps1
powershell -ExecutionPolicy Bypass -File scripts\capture.ps1 select           # Snip & Sketch -> clipboard
```

Notes for the Windows port:
- `window <hwnd>` / `app <name>` use `PrintWindow` (with `PW_RENDERFULLCONTENT`),
  which captures most hardware-accelerated windows; it falls back to an on-screen
  region copy if `PrintWindow` returns nothing.
- `app` matches the process name **or** the window title (case-insensitive
  substring) and brings the window to the foreground before capturing.
- `pick-window` (click a window) has no native non-interactive equivalent; use
  `select` (drag a region via Snip & Sketch) or `app <name>` / `window <hwnd>`.
- The macOS sections below describe `screencapture` specifics — on Windows the
  same modes are served by `capture.ps1`.
...
On **Windows** no special permission is needed. If `capture.ps1` is blocked by
execution policy, invoke it with `-ExecutionPolicy Bypass` (as shown above). If a
specific window comes back black, it's usually a window using an overlay/secure
surface — fall back to `full`, `display N`, or `region`.
